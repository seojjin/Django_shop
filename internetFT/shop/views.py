...
class postViewSet(viewsets.ModelViewSet):
    queryset = Item.objects.all() #검색 결과
...
def likes(request, item_id):
    like_b = get_object_or_404(Item, id=item_id)
    if request.user in like_b.like.all():
        ...
        like_b.like_count += 1
        like_b.save()
    return redirect('/shop/' + str(item_id))

class ItemUpdate(LoginRequiredMixin, UpdateView):
    model = Item
    fields = ['name', 'content', 'image', 'price', 'point', 'del_price', 'company', 'category']
